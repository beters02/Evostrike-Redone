...
-- LOCAL RUN VOLUME
local runv = 0.6
Movement.Sounds.runMetal.Volume = 0.6
Movement.Sounds.runTile.Volume = 0.6
...
Movement.moveMaxSpeedAdd = 0 -- (for walk/crouch speed)

-- store current max speed for easy access
Movement.currentGroundMaxSpeed = Movement.groundMaxSpeed
...
function Movement.Run(hitPosition, hitNormal, hitMaterial)
	if hitPosition.Y ~= -3000000 then lastSavedHitPos = hitPosition end
	Movement.movementPosition.position = lastSavedHitPos + Vector3.new(0, Movement.playerTorsoToGround, 0)
...
function Movement.Crouch(crouch: boolean)
	...
	if crouch then
		...
		-- slow player
		Movement.moveMaxSpeedAdd -= (Movement.groundMaxSpeed - Movement.crouchMoveSpeed)
...
function Movement.Walk(walk: boolean)
	if walk then
		...
		-- slow player
		Movement.moveMaxSpeedAdd -= (Movement.groundMaxSpeed - Movement.walkMoveSpeed)
...
local processCrouch
local processWalk
local lastStoredColliderVelocity

-- TEST DELETE ME
local printMovement = true

function Movement.ProcessMovement()

	-- unnatural collider velocity resolution test
	if not lastStoredColliderVelocity then lastStoredColliderVelocity = collider.Velocity end

	if collider.Velocity.Magnitude >= lastStoredColliderVelocity.Magnitude * 25 then
		collider.Velocity = lastStoredColliderVelocity
	end
	...
	local hitPart, hitPosition, hitNormal, hitMaterial = sharedMovementFunctions.IsGrounded(player)

	if not hitPart then hitPart, hitPosition, hitNormal = Movement:FindCollisionRay() hitMaterial = hitPart and hitPart.Material end
	playerGrounded = hitPart and true or false

	--local _test = sharedMovementFunctions.IsGrounded(player)
	
	--playerGrounded = hitPart and true or false

	playerVelocity = collider.Velocity - Vector3.new(0, collider.Velocity.y, 0)

	if UserInputService:IsKeyDown(Enum.KeyCode.F8) then print(hitPosition, playerVelocity, playerGrounded, Movement, jumping) end
	...
	if playerGrounded and inAir and (not Movement.jumpGrace or tick() >= Movement.jumpGrace) then
		...
		if tick() >= a + Movement.minInAirTimeRegisterLand and not landing then
			...
			Movement.Run(hitPosition, hitNormal, hitMaterial)
			return
		end
	end

	-- [[ JUMP INPUT REGISTRATION ]]
	...
	-- set ground max speed
	Movement.currentGroundMaxSpeed = math.min(
		Movement.groundMaxSpeed,
		Movement.groundMaxSpeed + Movement.maxSpeedAdd + Movement.moveMaxSpeedAdd
	)
...
function Inputs.OnInput(input) -- began and end
	...
	if input.KeyCode == Enum.KeyCode.F8 then
		printMovement = not printMovement
	end
