...
function Molly:FireGrenade(hit, origin, direction, thrower)
    ...
    if initresult then
        ...
        grenade.Position = Players.LocalPlayer.Character.HumanoidRootPart.Position + Vector3.new(0, self.Options.startHeight, 0)
        grenade.Velocity = startLv * 1.5
        grenade.Parent = workspace:WaitForChild("Temp")

        -- then we just send ray hit instead
        self:RayHit(false, initresult, startLv * 1.5, grenade)
        return
    end
...
function Molly:RayHit(_, result, velocity, grenade)
    local explode = true
    local touchedConn
...
function Molly.ServerExplode(position)
    ...
    local t = tick() + Molly.Options.mollyLength
    local conn
    local charsDamaging = {}
                ...
                if not charsDamaging[char.Name] then
                    charsDamaging[char.Name] = {nextDamageTick = tick(), connection = RunService.Heartbeat:Connect(function()
                        if tick() >= charsDamaging[char.Name].nextDamageTick then
                            ...
                            charsDamaging[char.Name].nextDamageTick = Molly.Options.damageInterval + tick()
                            char:SetAttribute("lastHitPart", "LeftFoot")
                            char:SetAttribute("lastUsedWeapon", "Ability")
                            EvoPlayer:TakeDamage(char, Molly.Options.damagePerInterval)
        ...
        if not visualizerPart:GetAttribute("IsDestroying") then
            ...
            task.spawn(function()
                for _, v in pairs(visualizerPart:GetChildren()) do
                    if v:IsA("Sound") then
                        TweenService:Create(v, TweenInfo.new(Molly.Options.mollyFadeLength), {Volume = 0})
                    end
                end
            end)

            task.delay(0.1, function()
                TweenService:Create(visualizerPart, TweenInfo.new(Molly.Options.mollyFadeLength), {Transparency = 1}):Play()
            end)

            if visualizerPart:FindFirstChild("Attachment") then
                for _, v in pairs(visualizerPart.Attachment:GetChildren()) do
                    TweenService:Create(v, TweenInfo.new(Molly.Options.mollyFadeLength), {Rate = 0}):Play()
                end
            end

            task.wait(Molly.Options.mollyFadeLength + 2)
