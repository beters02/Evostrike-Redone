...
--@override
function Satchel:FireGrenadePost(_, _, _, grenade)
    ...
    self.satchelConnection = RunService.RenderStepped:Connect(function()
        ...
        if UserInputService:IsKeyDown(Enum.KeyCode[self.Key]) and not blasted then
            blasted = true
            self:Blast(self.Player, grenade)
        end
    end)
...
function Satchel:Blast(thrower, grenade)
    ...
    local hrpPos = self.Player.Character.HumanoidRootPart.CFrame.Position
...
function Satchel.ServerPop(thrower, position)
    ...
    for _, plr in pairs(Players:GetPlayers()) do
        ...
        local radius = Satchel.Options.explosionDamageMaxRadius
        if distance <= radius then
            plr.Character:SetAttribute("lastHitPart", "Torso")
            EvoPlayer:TakeDamage(plr.Character, math.clamp(((radius - distance)/radius) * Satchel.Options.explosionMaxDamage, Satchel.Options.explosionMinDamage, Satchel.Options.explosionMaxDamage))
        end
    end
